...
test("core order reliability provides idempotency, inventory settlement and customer timelines", async () => {
  ...
  const inventory = await readFile(new URL("../lib/inventory-settlement.ts", import.meta.url), "utf8");
  ...
  assert.match(migration, /order_inventory_reservations/);
  assert.match(inventory, /available_quantity -/);
  assert.match(inventory, /reserved_quantity -/);
  assert.match(webhook, /"RESERVED", "CONSUMED"/);
  assert.match(webhook, /"CONSUMED", "RELEASED"/);
...
test("public Nexora presentation contains no AI platform credits", async () => {
  const page = await readFile(new URL("../app/page.tsx", import.meta.url), "utf8");
  const layout = await readFile(new URL("../app/layout.tsx", import.meta.url), "utf8");
  const footer = await readFile(new URL("../app/SupportPages.tsx", import.meta.url), "utf8");
  const publicPresentation = `${page}\n${layout}\n${footer}`;
  for (const credit of [/OpenAI/i, /ChatGPT/i, /Codex/i, /powered by AI/i, /generated by AI/i])
    assert.doesNotMatch(publicPresentation, credit);
  assert.doesNotMatch(page, />Ask Nexora</);
